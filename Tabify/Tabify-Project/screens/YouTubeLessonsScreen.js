      ...
      const loadUrl = async () => {
        try {
          ...
          if (url) {
            ...
            setDisplayUrl(url);
            await AsyncStorage.setItem("currentYouTubeLessonsUrl", url);
            setHasContent(true);
            console.log("YouTubeLessonsScreen - Set URL from navigation:", url);
          } else if (hasActiveSearch === "true" && storedUrl) {
            // Tab navigation - always use latest stored URL
            setDisplayUrl(storedUrl);
            setHasContent(true);
            console.log("YouTubeLessonsScreen - Set URL from AsyncStorage:", storedUrl);
          } else {
            setDisplayUrl(null);
            setHasContent(false);
            console.log("YouTubeLessonsScreen - No active search or URL, showing empty state.");
          }
          console.log(
            "YouTubeLessonsScreen - Final state - hasActiveSearch:",
            hasActiveSearch,
            "storedUrl:",
            storedUrl,
            "hasContent:",
  ...
  return (
    <WebView
      ...
      onError={(syntheticEvent) => {
        ...
        console.error("WebView error in YouTubeLessonsScreen:", nativeEvent);
      }}
    />
  );
};
